...
func (a *App) initializeRoutes() {
	repo := NewPostgresRepository(a.DB)
	service := NewService(repo)
	handler := NewHandler(service)

	a.Router = mux.NewRouter()
	a.Router.Handle("/movies", handler.GetMovies()).Methods("GET")
	a.Router.Handle("/movies/{id}", CheckMovieID(handler.GetMovie())).Methods("GET")
	a.Router.Handle("/movies/{id}/reviews", CheckMovieID(handler.GetMovieReviews())).Methods("GET")
	a.Router.Handle("/movies", handler.AddMovie()).Methods("POST")
	a.Router.Handle("/movies/{id}/reviews", CheckMovieID(handler.AddMovieReview())).Methods("POST")
}
